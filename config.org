# -*- after-save-hook: (org-babel-tangle); eval: (message "Welcome to ECFPAW's literate configuration."); -*-
...
#+STARTUP: content
#+PROPERTY: header-args :tangle init.el :noweb-ref no
* LICENSE NOTICE
  :PROPERTIES:
  :VISIBILITY: folded
  :END:
...
** TODO Completely overhaul the way I do keybindings.
...
This is using noweb references to get all code blocks for individual
keybindings under [[Keybindings]] inside the use-package call in
...
#+begin_src elisp :noweb yes
  (use-package general
    ...
    <<individual_keybindings>>)
#+end_src
** Keybindings
:PROPERTIES:
:header-args: :noweb-ref individual_keybindings :noweb no
:END:
...
#+begin_src elisp
  (defvar ECFPAW/themes (ECFPAW/make-cyclic-list
                         (list 'ef-frost 'ef-maris-dark 'modus-vivendi))
...
#+begin_src elisp
  ...
  (ECFPAW/def-mode-line-constr
   ...
   '(:eval
     (propertize
      (format "%i"
              (days-between "4 April 2024" (current-time-string)))
...
#+end_src
* ESHELL
I use Eshell most of the I need a shell inside Emacs. There is a
problem though, the ~clear~ command doesn't work like you would expect
it to. It turns out I need to alias it to ~clear-scrollback~!
