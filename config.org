...
#+begin_src elisp
  (ECFPAW/def-mode-line-constr ECFPAW/mode-line/title
      '(:eval
        (propertize "ECFPAW" 'face 'modus-themes-bold))
    "Mode line construct to display ECFPAW.")

  (ECFPAW/def-mode-line-constr ECFPAW/mode-line/jee-daily-countdown
      '(:eval
  ...
  (ECFPAW/def-mode-line-constr ECFPAW/mode-line/major-mode
      '(:eval 
        (propertize
         (capitalize (symbol-name major-mode)) 'face 'modus-themes-subtle-yellow))
    "Mode line construct to display the major mode.")

  (ECFPAW/def-mode-line-constr ECFPAW/mode-line/buffer-name
      '(:eval 
        (propertize (buffer-name) 'face 'modus-themes-nuanced-blue))
    "Mode line construct to display the buffer-name.")

  (ECFPAW/def-mode-line-constr ECFPAW/mode-line/time
      '(:eval 
        (propertize (current-time-string) 'face 'mode-line-emphasis))
    "Mode line construct to display the time")

  (ECFPAW/def-mode-line-constr ECFPAW/mode-line/toolbox-name
      '(:eval
        (propertize
         (substring (shell-command-to-string "cat /run/.containerenv | grep 'name='") 6 -2)
...
#+begin_src elisp
  (use-package dashboard
    ...
    (setq dashboard-items '((recents  . 5)
                            (projects . 5)))
    :config
    (push "What is like the org-mode? What can make war against it?"
