...
** TODO Make menu bar and mouse based menus obey the current theme.
...
#+begin_src elisp
    (use-package gptel
      :elpaca (:host github :repo "karthink/gptel" :branch "copilot")
      :config
      (setq-default gptel-model "gemma:2b" ;Pick your default model
                    gptel-backend
                    (gptel-make-ollama "Ollama"
                      :host "localhost:11434"
                      :stream t
                      :models '("orca-mini:latest"
                                "tinyllama:latest"
                                "gemma:2b"
                                "qwen:0.5b")))
...
#+begin_src emacs-lisp
  ...
  (use-package corfu-candidate-overlay
    ...
    (with-eval-after-load 'evil
      (evil-define-key 'insert corfu-mode-map
        (kbd "<tab>") #'corfu-candidate-overlay-complete-at-point)))
#+end_src
* TEXT
