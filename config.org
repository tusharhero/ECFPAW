...
*** Easy cycling
#+begin_src elisp
  (defvar ECFPAW/themes '(modus-vivendi vscode-dark-plus doom-gruvbox doom-gruvbox-light)
    "A list of the themes I like and use.")

  (defvar ECFPAW/current-theme nil
    "My current theme according to the cycling mechanism.")

  (defun ECFPAW/cycle-my-theme ()
    "Cycle through a list of themes, my-themes"
    (interactive)
    (when ECFPAW/current-theme
      (disable-theme ECFPAW/current-theme)
      (setq ECFPAW/themes (append ECFPAW/themes (list ECFPAW/current-theme))))
...
#+begin_src elisp
  ...
  (defun ECFPAW/mode-line/hide-on-hook (window)
    (with-current-buffer (if (windowp window) (window-bufr) (current-buffer)))
    (if (< (window-width window nil) 80)
        (setq-local mode-line-format ECFPAW/mode-line/mini-format)
      (setq-local mode-line-format ECFPAW/mode-line/full-format))
    (force-mode-line-update)
    "Change to mini mode line if the window is too small for the full one."
    )

  (add-hook 'window-size-change-functions  'ECFPAW/mode-line/hide-on-hook)

  ;; Faces
  (defface ECFPAW/mode-line/title-face
    '((t :background "black" :foreground "white" :box t :weight ultra-bold))
    "Mode line face title face for use on the mode line.")

  (defface ECFPAW/mode-line/major-mode-face
    '((t :background "midnight blue" :foreground "green yellow"
         :weight bold))
    "Mode line face for major mode for use on the mode line.")

  (defface ECFPAW/mode-line/buffer-name-face
    '((t :background "navy" :foreground "yellow"))
    "Mode line face for buffer name for use on the mode line.")

  ;; Constructs
  (defvar-local ECFPAW/mode-line/title
      '(:eval
        (propertize "ECFPAW" 'face 'ECFPAW/mode-line/title-face))
  ...
  (defvar-local ECFPAW/mode-line/major-mode
      '(:eval 
        (propertize (capitalize (symbol-name major-mode)) 'face 'ECFPAW/mode-line/major-mode-face))
    "Mode line construct to display the major mode.")
  (put 'ECFPAW/mode-line/major-mode 'risky-local-variable t)

  (defvar-local ECFPAW/mode-line/buffer-name
      '(:eval 
        (propertize (buffer-name) 'face 'ECFPAW/mode-line/buffer-name-face))
