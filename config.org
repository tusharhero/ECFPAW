...
:TODO:
+ [-] Things that need to implemented [1/4]
  + [ ] C-c C-c support for custom cookies
  + [X] Face support for custom cookies
  + [ ] Add better face support for custom cookies
...
#+begin_src elisp
  (use-package org-custom-cookies
    ...
    (font-lock-add-keywords 'org-mode '(("\\[D:[ ]*[0-9]*\\]" . 'org-checkbox-statistics-todo))))
#+end_src
*** Subheading counting
...
#+begin_src elisp
  (defun ECPFAW/org-custom-cookies--direct-descendant-subentries ()
    ...
    (format "[D: %s]" (ECFPAW/org-number-of-direct-descendant-subentries)))
#+end_src
* NEWS-TICKER
